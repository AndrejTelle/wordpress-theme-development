...
module.exports = function(grunt) {
    ...
    grunt.initConfig({
        concat: {
            dev: {
                src: appScriptsNoTests,
                dest: 'TimeTracker.Web/app/build/dev.js'
            }  
        },
        ngAnnotate: {
            options: {
            ...
            dev: {
                files: {
                    'TimeTracker.Web/app/build/dev.js': appScriptsNoTests
                }
            },
            live: {
                files: {
                    'TimeTracker.Web/app/build/live.js': appScriptsNoTests
                }
            }
        },
        watch: {
            // compile sass files
            css:{
                files: ['TimeTracker.Web/Styles/**/*.scss'],
                tasks: ['nodesass:all']
            },
            js: {
                files: appScriptsNoTests,
                tasks: ['ngAnnotate:dev'],
                options: {
                    /* interrupt a running task to start a new one if triggered during execution */
                    interrupt: true
                }
            }
        },
        nodesass: {
            ...
            all: {
                ...
                src: ['TimeTracker.Web/Styles/timetracker.scss']
            }
        }
    });
